...
| B&K Precision 9174B DC power supply behind CP2102 USB-UART | `0x10c4:0xea60` | `BK9174B` | USB-serial (`/dev/ttyUSB0`) |
...
The CP2102-connected DC supply is reached over `/dev/ttyUSB0` via pyserial (lazily imported by `instruments.BK9174B`; `pyserial` is in `requirements.txt`).
...
A run directory is what the SLDEA tab writes: `data.csv` (needs the `frame_file`, `tag` and `nominal_kV` columns) plus `frames\`, and optionally `setup.txt`, `run.log` and `telemetry.csv`. `telemetry.csv` is the continuous monitor log — one row per watchdog sample (~2 Hz), with `t_s`, `nominal_kV`, `measured_kV`, `measured_uA` and a status column per monitor, plus event rows marking each snapshot and any breakdown trip. It is a **sidecar**: nothing reads it to measure anything, `data.csv` is still the run, and a run recorded before this existed (or with the telemetry box unticked) is complete without it. The CSV may also be named `data1.csv`, `data2.csv` and so on — Excel refuses to open two workbooks with the same filename, so renaming copies is normal, and every reader here accepts it (an exact `data.csv` wins if both are present). Copy the folder to local disk before tuning — **Save writes the tuned values into that run's `setup.txt`**, and every slider drag re-reads the frames, which is slow over SMB.
...
- **Stepped capture** steps a signal-generator parameter (CH1/CH2 DC offset or amplitude) from start→stop by a step, dwells, and captures a frame at each value — useful for characterizing something optical vs. drive level. It can also log a `*_focus.csv` of the focus score per step. (The sweep drives the signal generator; the BK 9174B supply has its own tab and is not part of stepped capture.) **A sweep and an SLDEA run never write the same SG channel:** the SLDEA tab's ▶ Run refuses to start while a sweep is writing its channel (or while a timed capture or camera adjustment is running), and only asks when the sweep is on the other channel. A sweep also stops, rather than write, when it reaches a level on the channel a LIVE run owns.
